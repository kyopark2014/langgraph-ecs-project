...
[installer.py](./installer.py)는 OpenSearch·Knowledge Base·VPC·ALB·ECS·CloudFront와 함께 **AgentCore Web Search Gateway**를 `us-east-1`에 프로비저닝합니다. Gateway는 애플리케이션 리전(`us-west-2`)과 분리된 **고정 리전**에 생성됩니다.
...
ECS 태스크 역할(`role-ecs-for-agent-skills-us-west-2`)에는 `bedrock-agentcore:*` 정책이 포함되어 컨테이너 런타임에서 Gateway MCP를 호출할 수 있습니다.
...
| 5/9 | Knowledge Base | — |
| 6~9 | VPC, ALB, CloudFront, ECS | `build_app_environment()`로 config 반영 |
...
성공 시 로그에 Gateway ID·URL·역할 ARN이 출력되고, `application/config.json`에 아래 필드가 기록됩니다. ECS 배포 시에도 동일한 값이 컨테이너 환경에 전달됩니다.
...
로컬 개발·EC2·ECS 컨테이너 모두 **실행 환경의 AWS credential**(프로파일, instance role, task role 등)이 Gateway 호출 권한을 가져야 합니다.
...
`aws configure` 또는 EC2/ECS task role로 credential을 설정한 뒤 Agent를 실행합니다.
