...
`installer.py`가 수행하는 작업 (Memory·Session Storage·ECS 관련):
...
3. `application/config.json`에 `agentcore_memory_role`, `memory_id`, `sharing_url`, S3 Files 키 등 저장
4. VPC, ALB, **S3 Files 세션 스토리지** (`[5.5/10]`), ECS Fargate(S3 Files `/mnt/workspace` 마운트, `SESSION_STORAGE_DIR` 환경 변수), Knowledge Base, AgentCore Web Search Gateway, CloudFront 등 인프라 배포
5. 기존 ECS 서비스가 있으면 `update_service`로 task definition·**networkConfiguration**(subnet·SG)·**loadBalancers**(target group)를 함께 갱신
...
Agent / Agent (Chat) 응답 완료 + Memory Enable
...
| `sharing_url` | CloudFront 배포 URL (`https://{distribution}.cloudfront.net`) |
| `s3_files_file_system_id` | S3 Files file system ID (LangGraph checkpoint 영속화) |
| `s3_files_access_point_arn` | S3 Files access point ARN |
| `s3_files_mount_path` | ECS 컨테이너 마운트 경로 (기본 `/mnt/workspace`) |
| `ecs_session_vpc_subnets` | S3 Files mount target이 연결된 private subnet ID 목록 |
| `ecs_session_security_groups` | ECS 태스크·S3 Files NFS용 security group ID 목록 |
...
기존 `MemorySaver`는 프로세스 메모리에만 저장되어 컨테이너가 재시작되면 history가 사라집니다. **Agent (Chat)** 모드(`history_mode=Enable`)일 때 [application/app.py](./application/app.py)가 요청마다 `asyncio.run(langgraph_agent.run_langgraph_agent(...))`을 호출하고, [application/chat.py](./application/chat.py)의 `ensure_checkpointer()`가 **AsyncSqliteSaver**를 초기화합니다. [application/langgraph_agent.py](./application/langgraph_agent.py)의 `buildChatAgentWithHistory()`가 이를 checkpointer로 사용합니다.

| 모드 | `history_mode` | checkpointer | 대화 이력 |
|------|----------------|--------------|-----------|
| **Agent** | `Disable` | 없음 (stateless) | 턴마다 초기화 |
| **Agent (Chat)** | `Enable` | AsyncSqliteSaver + S3 Files | User ID(`thread_id`)별 유지 |
...
#### Streamlit + `asyncio.run()` 이벤트 루프

ECS Streamlit 앱은 요청마다 `asyncio.run()`으로 **새 이벤트 루프**를 만들고 종료합니다. `AsyncSqliteSaver` 내부 `asyncio.Lock`은 생성 시점 루프에 바인딩되므로, 전역 캐시를 그대로 재사용하면 두 번째 메시지부터 `RuntimeError: Lock ... is bound to a different event loop`가 발생할 수 있습니다.

이를 방지하기 위해 구현은 다음과 같습니다.

1. **`chat.ensure_checkpointer()`** — `_checkpointer_loop`로 현재 루프를 추적하고, 루프가 바뀌면 기존 SQLite 연결을 닫고 checkpointer를 재생성합니다. 초기화 동기화는 `threading.Lock()`을 사용합니다. `langgraph-checkpoint-sqlite` 미설치·SQLite 오류 시 `MemorySaver`로 폴백합니다.
2. **`langgraph_agent.run_langgraph_agent()`** — MCP·Skill·User ID·`history_mode` 변경 시, 또는 `history_mode=Enable`에서 이벤트 루프 ID(`active_loop_id`)가 바뀌면 캐시된 LangGraph `app`을 폐기하고 `create_agent()`로 재빌드합니다.
3. **요청 종료 시** — `persist_checkpoint_to_session_storage()`로 working DB를 `/mnt/workspace/{user_id}/`에 복사하여 다음 요청(새 루프)에서 `_restore_from_session_storage()`로 복원합니다.

```python
# application/chat.py — ensure_checkpointer() 요약
loop = asyncio.get_running_loop()
if _sqlite_checkpointer is not None and _checkpointer_loop is not loop:
    await _close_sqlite_checkpointer()   # 루프 변경 시 재생성
...
```python
# application/langgraph_agent.py — run_langgraph_agent() 캐시 무효화
...
사이드바 **대화 초기화** 버튼은 `chat.reset_checkpoint_state()`를 호출하여 메모리·checkpoint 상태를 초기화합니다 ([application/app.py](./application/app.py)).
...
sequenceDiagram
    ...
    UI->>LG: asyncio.run — Agent (Chat), user_id
    LG->>CP: ensure_checkpointer() (루프 변경 시 재생성)
...
`SESSION_STORAGE_DIR` 해석 순서 ([application/chat.py](./application/chat.py)의 `_resolve_session_storage_dir()`):

1. 환경 변수 `SESSION_STORAGE_DIR` (ECS task definition에서 설정)
2. `config.json`의 `s3_files_mount_path`
3. `/mnt/workspace` 디렉터리 존재 시 해당 경로
4. 로컬: `application/.session_storage`

로컬 개발 시 `/mnt/workspace`가 없으면 checkpoint durable 경로는 `application/.session_storage/{user_id}/`입니다.
...
9. **기존 ECS 서비스 재배포** — 서비스가 이미 있으면 `update_service`로 task definition·`networkConfiguration`·`loadBalancers`를 함께 갱신. `services_stable` 대기 실패 시 `_log_ecs_service_diagnostics()`가 ECS 이벤트·stopped task 원인을 로그에 출력
...
Docker 이미지를 갱신한 경우 installer가 ECR push 후 ECS service를 `forceNewDeployment`합니다. 컨테이너 시작 시 [docker-entrypoint.sh](./docker-entrypoint.sh)가 ECS task definition의 `APP_CONFIG_JSON` 환경 변수를 `/app/application/config.json`으로 씁니다.
...
- **트러블슈팅**
  ...
  - **Agent (Chat)** 두 번째 메시지에서 `Lock ... is bound to a different event loop` → `chat.py`·`langgraph_agent.py`의 이벤트 루프 재생성 로직이 포함된 최신 이미지로 재배포
  - ECS `Waiter ServicesStable failed` → installer 진단 로그·CloudWatch에서 `InvalidSubnetID` / `InvalidGroup` / 삭제된 target group 참조 여부 확인 후 `installer.py` 재실행
  - CloudWatch `/ecs/app-for-{project}` 로그에서 `SQLite checkpointer`, `Event loop changed; recreating SQLite checkpointer` 메시지 확인
...
| **Agent** | `langgraph_agent.run_langgraph_agent` | LangGraph StateGraph + built-in tools + MCP + Skills. `history_mode=Disable`, checkpointer 없음 (단일 턴) |
| **Agent (Chat)** | `langgraph_agent.run_langgraph_agent` | Agent와 동일 + **AsyncSqliteSaver** checkpointer. `history_mode=Enable`, `thread_id` = `user_id`, S3 Files `/mnt/workspace`에 영속화. Streamlit은 요청마다 `asyncio.run()` 사용 |
...
- Streamlit은 요청마다 `asyncio.run()`으로 새 이벤트 루프를 사용하므로, checkpointer·LangGraph `app`은 루프 변경 시 재생성되고 checkpoint 파일로 이력이 이어집니다.
- 요청 종료 시 `persist_checkpoint_to_session_storage()`가 working DB를 `/mnt/workspace/{user_id}/`로 복사합니다.
- CloudWatch(`/ecs/...`) 또는 애플리케이션 로그에서 `trimmed messages from X to Y (max_turns=5)`, `SQLite checkpointer opened (existing)`, `Event loop changed; recreating SQLite checkpointer` 메시지를 확인할 수 있습니다.
...
설치가 완료되면 **CloudFront URL**(`sharing_url` in `config.json`)로 접속하여 Agent를 실행합니다. 앱 시작 시 **User ID**를 입력하고, 사이드바에서 **Memory** 및 MCP(`short term memory`, `long term memory`)를 선택합니다. **Agent (Chat)** 모드에서 User ID별 LangGraph checkpoint가 S3 Files(`/mnt/workspace`)에 영속화됩니다. 애플리케이션 코드 변경 후에는 Docker 이미지 재빌드·ECR push가 필요하며, `installer.py`가 ECS `forceNewDeployment`까지 수행합니다.
...
[installer.md — 인프라 상세](./installer.md)

[uninstaller.py — 인프라 삭제](./uninstaller.py)
